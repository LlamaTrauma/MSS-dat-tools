...
class Bone():
    def __init__(self, id, GEOID, orientation, children, parent, relative, track_id):
        self.id = id
        self.GEOID = GEOID
    ...
    def addInfluence (self, influence):
        ...
        if vertex in self.vertexInfluences:
            ...
            if sum([influence.source_position[i] != self.vertexInfluences[vertex][1][i] for i in range(3)]) > 0:
                print('    initial position set differently')
        self.vertexInfluences[vertex] = [influence.weight, influence.source_position]
...
class ACTLayout(FileChunk):
    ...
    def bones(self):
        ...
        for bone_addr in self.bone_layouts:
            ...
            bone_obj = Bone(bone.id, bone.geoFileId, bone.orientation_srt, [], None, bone.inheritance, bone.track_id)
...
# The influence a bone has on a vertex. Parameters are bone id, weight (/255), the address of a vertex array in the gpl this vertex is a part of, and the index in that array
...
class BoneInfluence():
    def __init__(self, boneID, weight, vertexIndex, source_position):
        self.boneID = boneID
        self.weight = weight
...
class SK1(FileChunk):
    ...
    def boneInfluences(self):
        ...
        boneInfluences = [BoneInfluence(self.boneIndex, 255, (self.gplVertexArr + self.vertexOffset) // vertexSize + i, position_normal_data[i * 2]
...
class SK2(FileChunk):
    ...
    def boneInfluences(self):
        ...
        boneInfluences = []
        for i in range(self.vertexCnt):
            boneInfluences.append(BoneInfluence(self.boneIndex1, self.weights[i][0], (self.gplVertexArr + self.vertexOffset) // vertexSize + i, position_normal_data[i * 2]))
            boneInfluences.append(BoneInfluence(self.boneIndex2, self.weights[i][1], (self.gplVertexArr + self.vertexOffset) // vertexSize + i, position_normal_data[i * 2]))
        return boneInfluences
...
class SKAcc(FileChunk):
    ...
    def boneInfluences(self):
        ...
        boneInfluences = [BoneInfluence(self.boneIndex, self.weights[i], self.gplDestArr // vertexSize + self.dests[i], position_normal_data[i * 2]) for i in range(self.vertexCnt)]
        return boneInfluences
