...
class ANMTrack(FileChunk):
    ...
    def analyze(self):
        ...
        #     self.keyframes = []
        #     return self
        self.keyframes = [self.add_child(self.keyframe_arr_ptr + 12 * i, 0, ANMKeyframe, relative=False).analyze() for i in range(self.keyframe_cnt)]
        return self
...
class ANMSequence(FileChunk):
    def analyze(self):
        ...
        self.pad = self.half()
        self.tracks = [self.add_child(self.track_arr_ptr + 16 * i, 0, ANMTrack, relative=False).analyze() for i in range(self.track_cnt)]
...
class ANM(FileChunk):
    def analyze(self):
        ...
        self.sequences = [self.add_child(self.sequence_arr_ptr + 12 * i, 0, ANMSequence, relative=False).analyze() for i in range(self.sequence_cnt)]
